    ...
    <div class="row">
      ...
      <div class="col-lg-5 col-md-5">
        <div class="d-flex justify-content-center">
          <label for="inputState" style="font-weight: bold;">Filtro por usuario</label>
        </div>
        <div class="d-flex justify-content-center">
          <a-select  default-value="Selecciona un proyecto" style="width: 398px" @change="handleChange" placeholder="Selecciona un proyecto">
            ...
            <a-select-option  v-for="(users) in users" :key='users.id'>
              {{users.name}}
            </a-select-option>
            template
          </a-select>
          ...
          <div class="col-lg-6 col-md-6">
            ...
            <table class="table">
              <thead style="background: #f4f7f9" >
                <th style="font-weight: bold;">Coolaborador</th>
...
@Component
export default class ReportsByUser extends Vue {
  ...
  async initialInfo(){
    ...
    async recordsbyProject(){
      ...
      const query = await axios.get('/api/records-user');
    ...
    async  dateRange(date: any) {
      ...
      const graphStructure = [];
      const filter = await axios.post('/api/datefilter-user',infoFilter);
      const detailFilter = await axios.post('/api/detail-filter-user',infoFilter);
